...
def read_vcf(vcf_file, chm=None, fields=None, verbose=False, samples=None):
    """
    ...
    if vcf_file[-3:]==".gz":
        with gzip.open(vcf_file, 'rb') as vcf:
            data = allel.read_vcf(vcf,  region=chm, fields=fields, samples=samples)
    else: 
        data = allel.read_vcf(vcf_file, region=chm, fields=fields, samples=samples)
